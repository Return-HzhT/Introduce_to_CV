...
def bn_forward_train(x, gamma, beta):
    ...
    x_hat = x - sample_mean
    sample_var = 1.0 / n * np.sum((x_hat**2), axis=0)
    x_hat = x_hat / np.sqrt(sample_var + 1e-3)
    out = gamma * x_hat + beta
    #----------------TODO------------------
...
def bn_backward(dout, cache):
    ...
    dvar = np.sum(dx_hat * (x - sample_mean) * (-0.5) *
                  ((sample_var + 1e-3)**(-1.5)),
                  axis=0)
    dmean = dvar * (1.0 / n) * (-2) * np.sum(x - sample_mean, axis=0) + np.sum(
        dx_hat * (-1) / np.sqrt(sample_var + 1e-3), axis=0)
    dx = dx_hat / (np.sqrt(sample_var + 1e-3)) + dvar * 2 * (
        x - sample_mean) / n + dmean / n
...
if __name__ == "__main__":
    ...
    for i in range(50):
        ...
        if i > 0:
            mean = 0.05 * mean + 0.95 * cache[2]
            var = 0.05 * var + 0.95 * cache[3]
        else:
            mean = cache[2]
            var = cache[3]
