...
class CIFAR10(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if rand_num == 1:
            img = cv2.flip(img, 1).astype(np.float32)
        elif rand_num == 2:
            img = cv2.convertScaleAbs(img, alpha=0.7).astype(np.float32)
        elif rand_num == 3:
            img = cv2.convertScaleAbs(img, alpha=1.3).astype(np.float32)
        return img, target
...
if __name__ == '__main__':
    ...
    # --------------TODO------------------
    aug2 = np.asarray(img)
    aug2 = cv2.convertScaleAbs(aug2, alpha=0.7)
